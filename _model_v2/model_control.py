...
def new_scenario():
    ...
    return local_scenario_folder, network_scenario_folder
...
def initialize():
    '''Creates a new model scenario.
    Returns:
        local_scenario_folder, network_scenario_folder (tuple) - local and network folders to save the model

    '''
    local_scenario_folder, network_scenario_folder = new_scenario()
    _q = input('\n| Run flat oil pricing? Y/N >>> ')
    if _q.lower() == 'y':
        get_flat_oil_price()
    else:
        print('>>> Strip pricing will be used for oil.')
    _q = input('| Run flat gas pricing? Y/N >>> ')
    if _q.lower() == 'y':
        get_flat_gas_price()
    else:
        print('>>> Strip pricing will be used for gas.')
